...
type
  TRequestTypeCall = class(TInterfaceMenuCreate)
  public
...
begin
  ...
  fFileCreate.OptionsFMain(FMain, RTypeIni,'FMain_');
  fFileCreate.OptionsDBGrid(FMain.DBGrid1, RTypeIni,'DBGrid_');
  FMain.ADOQuery1.Close;
  FMain.GetPanel1;

  ListLabels:=TList<TLabel>.create;
  fFileCreate.OptionsLabels(AOwner,RTypeIni,'Label_',1);

  ListButtons:=TList<TButton>.create;
  fFileCreate.OptionsButtons(AOwner,RTypeIni,'Button_',1);
  ListButtons.Items[0].OnClick:=Button1Click;
  ListButtons.Items[1].OnClick:=Button2Click;

  ListComboBoxs:=TList<TComboBox>.create;
  fFileCreate.OptionsComboBoxs(AOwner,RTypeIni,'ComboBox_',1);
end;
...
begin
  ...
  begin
    SQL.Add('WHERE typecall='+''''+ListComboBoxs.Items[0].Items[ListComboBoxs.Items[0].ItemIndex]+'''');
...
begin
  ...
  begin
    ...
    SQL.Add('RECONFIGURE');}
    ExFileName:=getcurrentdir()+'\Îò÷åòû\'+
